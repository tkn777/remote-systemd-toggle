...
copy_sources() {
	...
	cp -R "$ROOT_DIR/systemd-service-toggle" "$SRC_DIR/"
	cp -R "$ROOT_DIR/systemd-service-toggled" "$SRC_DIR/"
...
set_source_version() {
	...
	sed -i "s/var version = \"[^\"]*\"/var version = \"$version\"/" \
		"$SRC_DIR/systemd-service-toggle/main.go" \
		"$SRC_DIR/systemd-service-toggled/main.go"
}

package_unit_file() {
	unit_file="$BUILD_DIR/systemd-service-toggled.service"
	cp "$ROOT_DIR/systemd-service-toggled.service" "$unit_file"
	sed -i 's#/usr/local/bin/systemd-service-toggled#/usr/bin/systemd-service-toggled#' "$unit_file"
	printf '%s\n' "$unit_file"
}
...
for arch in amd64 arm64; do
	...
	build_deb "$CLIENT_NAME" "$client_bin" "systemd-service-toggle" "$arch" \
		"mTLS-protected remote systemd service toggle client" "no"
	build_deb "$SERVER_NAME" "$server_bin" "systemd-service-toggled" "$arch" \
